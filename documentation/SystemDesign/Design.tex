...
The Computer Visions subsystem will be a service to the Smart Serve subsystem to determine if a shot is successfully returned by the user or not. When sent a request to begin tracking the ball, it will do so and return a true or false when the ball bounces off the table or it doesn't. In the event the ball never enters frame, it will assume the return was failed after a fixed amount of time. This subsystem will also aid in further analyzing the return made by the player.

\subsubsection{Shot Recommendation}
The Shot Recommendation subsystem will be a service to the Smart Serve subsystem to determine which shot should be taken next. The way the subsystem decides on the shot will be determined by the training mode the user selects from the user interface. These will include shooting the same shot every time, pseudo-randomly picking a shot or using reinforcement learning algorithms to decide the next shot. The last mode will be based on previous performance by the user.

\subsubsection{Shooting Model}
The Shooting Model subsystem will be a service to the Smart Serve subsystem which provides the means of mapping a desired shot to the details needed to take the shot. The input and output for the system can be found in Figure \ref{fig:shotmodel}. Essentially this system will perform all the physics calculations needed to shot the ball at the system desired location. The output will be many combinations of pitch, yaw, speed and angular velocity that satisfy the input shot details. 
\begin{figure}[htbp]
   \centering
...
The Shot Optimizer will provide means of reducing the travel time the shooting mechanism needs to do in order to shoot the desired shot (optimizer). This will simply scan each possible orientation and pick one which minimizes the distance travelled for the mechanism.
...
The Shooting Mechanism will be the subsystem which actually shoots the ball towards the user side from the system side. A microcontroller possibly Arduino Uno will be used as the point of contact and set any actuators required to shoot a specified shot. All of the hardware will be solely controlled by the microcontroller and will be the primary interface between the Smart Serve system and its electromechanical actuators. It must complete this action within 1.5 seconds for the Computer Vision to detect the ball returned by the user. 
